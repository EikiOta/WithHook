...
export default function WordDetailTabs({
  ...
  const handleSaveToMyWords = async () => {
    // wordRecord が null でもサーバーアクション側で再検索するケースはあるが、
    // ここでは念のためチェック
    if (!wordRecord) {
      alert("単語レコードが存在しません。意味を登録してください。");
      return;
    }
                    ...
                    >
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (
                            selectedMemoryHook?.memory_hook_id === h.memory_hook_id
                          ) {
